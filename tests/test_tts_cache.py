...
class TestSegmentKey:
    @given(engine=_texts, voice=_texts, text=_texts)
    def test_key_is_stable(self, engine: str, voice: str, text: str) -> None:
        assert cache.segment_key(engine, voice, text) == cache.segment_key(engine, voice, text)
